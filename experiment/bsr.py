...
def configureExperiment(lenExp):
    ...
    articles = articles[:len(treatments)+1]
    readEnglishText = articles.pop()

    '''add preliminary tasks'''
    prelimTasks = [{'func' : blink5times, 'params' : ()},{'func' : restingStateOpenEyes, 'params' : ()}, {'func' : restingStateClosedEyes, 'params' : ()}, {'func' : doMath , 'params' : ()}, {'func' :readTextEnglish2 , 'params' : ()}]
...
def runExperiment(lenExp=3):
    ...
    for i,f in enumerate(config):
        ...
        MW.qualityCheckLoop() # check signal quality
...
if __name__ == '__main__':
    ...
    #runExperiment2(preliminaryTasks=True,randomTreatment=True, finalQuestions=True)
